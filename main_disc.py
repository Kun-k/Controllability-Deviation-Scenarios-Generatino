...
def main(argv):
    ...
    LR = 5e-4
    EPOCH = 32
    disc = Discriminator(state_dim=num_state, action_dim=num_action_adv, lr=LR, epoch=EPOCH, device=FLAGS.device)
        ...
        if FLAGS.adv_policy == "RL":
            for epoch in trange(FLAGS.n_epochs_adv):
                ...
                with Timer() as disc_timer:
                    if (epoch + 1) % FLAGS.disc_period == 0:
                        loss_disc, expert_acc, policy_acc = disc.update(replay_buffer_real, replay_buffer_sac)
                        print("loss_disc: ", loss_disc, "expert_acc: ", expert_acc, "policy_acc: ", policy_acc)
